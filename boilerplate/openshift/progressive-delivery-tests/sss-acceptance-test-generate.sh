...
usage() {
    cat <<EOF
    Usage: $0 "OPERATOR_NAME""
    OPERATOR_NAME is the name of the operator onboarding to the SSS progressive delivery pipeline
EOF
    exit -1
}
...
cat <<EOF > /hack/test/sss-acceptance-test.yaml
...
- name: SERVICE_ACCOUNT
  ...
  displayName: ${SERVICE_ACCOUNT} service account
